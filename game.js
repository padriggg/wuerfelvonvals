...
const material1 = new THREE.MeshStandardMaterial({ color: 0xFF6347, roughness: 0.4});
...
function addStar() {
  ...
  const material = new THREE.MeshStandardMaterial({
    map: moonTexture, normalMap: normalTexture, roughness: 0.9
  });
...
const box=[
  new THREE.MeshStandardMaterial({ color:0xffffff, map: new THREE.TextureLoader().load('jeff.png') }),
  new THREE.MeshStandardMaterial({ map: addVideo() }),
  new THREE.MeshStandardMaterial({ color:0xffffff, map: new THREE.TextureLoader().load('jeff.png') }),
  new THREE.MeshStandardMaterial({ color:0xffffff, map: new THREE.TextureLoader().load('click.png') }),
  new THREE.MeshStandardMaterial({ color:0xffffff, map: new THREE.TextureLoader().load('jeff.png') }),
  new THREE.MeshStandardMaterial({  map: addVideo() }),
]
...
function animate() {
  ...
  stars.forEach((star, index) => {
      ...
      star.position.y = dist * Math.abs(Math.cos(1/(Math.sqrt(dist))*step)) - ebeneY;
